...
async fn main() -> Result<()> {
    ...
    let test_spec = VideoSpec {
        title: "Text Overlay Demo".to_string(),
    ...
    println!(
        ...
        validation.correct_dimensions
    );
    println!(
        "  Duration in range (10-60s): {}",
        validation.duration_in_range
    );
    println!(
        "  File size under limit (<50MB): {}",
        validation.file_size_under_limit
    );
